...
class Etheme_Sales_Booster_Estimated_Delivery {
	...
	public function add_actions() {
		...
		add_shortcode('etheme_sales_booster_estimated_delivery', array($this, 'shortcode_output'));
	}
	...
	public function set_settings($custom_settings = array()) {
        ...
        if ( !$this->args['is_admin'] && count( $this->settings['only_for'] ) ) {
	...
	public function output() {
		...
		<<?php echo esc_attr($this->args['tag']) ?> class="sales-booster-estimated-delivery">
            <?php echo esc_html($this->settings['text_before']); ?>
            <?php $this->calculated_date(); ?>
	...
	public function calculated_date() {
...
//		$today = strtotime(get_gmt_from_date( gmdate( 'Y-m-d H:i', strtotime('today') + ( get_option( 'gmt_offset' ) * HOUR_IN_SECONDS ) ) . ':00'));
	    ...
	    if ( $this->settings['date_type'] == 'range') {
	        if ( $this->settings['days_type'] == 'number' )
	            $html .= $this->settings['min_days'] . ' - ' . $this->settings['max_days'] .' ' . esc_html__('days', 'xstore');
	        else
		        $html .= $this->calculate_week_day($today, absint($this->settings['min_days']), $this->settings['non_working_days'] ) . ' - ' . $this->calculate_week_day($today, absint($this->settings['max_days']), $this->settings['non_working_days'] );
	    }
	    else {
		    if ( $this->settings['days_type'] == 'number' )
			    $html .= $this->settings['days'] . ' ' . esc_html__('days', 'xstore');
		    else
			    $html .= $this->calculate_week_day($today, absint($this->settings['days']), $this->settings['non_working_days'] );
		}
