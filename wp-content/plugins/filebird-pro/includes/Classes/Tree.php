...
class Tree {
	...
	public static function getFolders( $order_by = null, $flat = false ) {
		...
		$tree = array();
		if( $flat === true ) {
			//TODO update later (this case is used in ajaxGutenbergGetFolder function)
			$tree            = self::getTree( $folders_from_db, 0, $flat, 0, true, $folder_colors, $folder_default_color );
		} else {
	...
	private static function getTree( $data = array(), $parent = 0, $flat = false, $level = 0, $show_level = false, $folder_colors = array(), $folder_default_color = '#8f8f8f' ) {
		$tree = array();
		foreach ( $data as $k => $v ) {
			if ( $v->parent == $parent ) {
